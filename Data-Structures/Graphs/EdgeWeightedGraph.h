...
public:
   ...
   int uno() const { return v; } // devuelve el vertice v --> primer argumento al formar la arista valorada
   int otro(int vert) const { return (vert == v) ? w : v; } // devuelve w si recibe por argumento v
   Valor valor() const { return _valor; } // devueleve el valor de la arista
   void print(std::ostream& o = std::cout) const {
      o << "(" << v << ", " << w << ", " << _valor << ")";
   }
private:
